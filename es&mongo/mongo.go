...
type Message struct {
	Content    string    `gorm:"not null;type:text" json:"content" bson:"id"`
...
func ListMessage(userID, toUserID uint64, msgTime int64) ([]Message, error) {
	newMsgTime := time.UnixMilli(msgTime)
	msgs := make([]Message, 0)

	// 构建排序规则
	sort := bson.D{{Key: "createtime", Value: -1}}
	// 这里是不是可以用联合索引 create_time
	cursor, err := MessageClinet.Find(context.TODO(), bson.M{
		"fromuserid": bson.M{"$in": []uint64{userID, toUserID}},
		"touserid":   bson.M{"$in": []uint64{userID, toUserID}},
		"createtime": bson.M{"$gt": newMsgTime},
